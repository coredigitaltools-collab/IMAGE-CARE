...
## Bills & Payables (IMC-SRS-010), the Finance-side view of data that already existed

- **"Bills originate from supplier invoices" is implemented literally, not just followed as a rule.** A Bill *is* a `SupplierInvoice` (the record already created in Purchasing, IMC-SRS-007), there's no separate Bill entity duplicating amount, status, or payment history. `billsService.ts` is a thin layer over the exact same data and reuses `recordInvoicePayment()` rather than re-implementing "a payment reduces the balance" a third time in this app (Credit Management and Invoices each already implement that once). Verified directly: recorded a supplier invoice under Purchasing → Invoices, confirmed it immediately appeared in the Payables Register with no separate "sync" step, because it's the same row in the same table.
- **The full lifecycle was extended, not rebuilt**: `SupplierInvoiceStatus` gained `cancelled` and `closed` (previously only `unpaid`/`partially_paid`/`paid`), completing the spec's explicit workflow ending in "Closed" as a distinct step from merely being paid off. Guards were added at the service layer, not just the UI: a paid or closed bill can't be cancelled, only a fully paid bill can be closed, and a cancelled or closed bill can no longer accept a payment (verified: the "Record payment" action is genuinely unavailable, not just visually hidden, the underlying function throws if called anyway).
- **Aging Analysis mirrors Credit Management's aging report** (same bucket structure: Not yet due / Current / 31-60 / 61-90 / 90+ days overdue) for a consistent mental model across both sides of the business, money owed *to* the business vs money the business owes. Verified: a bill with a deliberately past due date correctly landed in the right overdue bucket.
- **Supplier Statements are a real running-balance reconstruction** from actual bills and payments (bills as debits, payments as credits, chronologically ordered), not a static list, verified showing a bill as a debit entry for a specific supplier.
- **Existing pages were updated where the new statuses now apply**: Purchasing's own Supplier Invoices list (which predates this module) needed its status-color mapping extended and its "Pay" button guarded against showing for a cancelled or closed bill, a real gap TypeScript itself caught immediately when the shared type was extended, not something found by accident later.
- **Settings stays honest about where configuration actually lives**: due dates and payment terms are set when a bill is created (in Purchasing), not duplicated into a second settings surface here, the Bills Settings page says so directly rather than pretending to own configuration it doesn't.

## Payroll (IMC-SRS-011), employees come from Staff Master, not a duplicate roster

- **"Employees come from Staff Master" is implemented literally.** A `PayrollEmployeeRecord` references an existing `StaffMember` (Settings → People & Access) by ID, it adds compensation data (base salary, assigned allowances/deductions), it never duplicates name, role, or branch. Same pattern as Bills referencing Supplier Invoices and Invoices referencing Sales, applied to a genuinely new domain this time.
- **"No hard coded payroll data"**, Allowances and Deductions are entirely business-defined types (name, and either a fixed UGX amount or a percentage of base salary), with an optional per-employee override. Nothing assumes a specific country's tax code or a preset list.
- **"Payroll periods cannot overlap" is enforced, not just described**, verified directly: created one period, then tried creating a second one covering the same dates, and it was correctly rejected.
- **"Processed payroll is locked after approval" is a real guard, not a UI convention.** Calculation can be re-run freely while a period is still in draft (to fix a mistake before committing), but the moment it's approved, the Recalculate button is genuinely gone and the underlying function refuses to run, verified by confirming both.
- **The calculation itself was verified with real numbers**, not just code review: a 1,000,000 UGX base salary, a 50,000 UGX fixed allowance, and a 5%-of-base deduction correctly produced a net pay of exactly 1,000,000 UGX (1,050,000 gross − 50,000 deduction), confirmed on the actual payslip.
- **The full workflow was run start to finish**: Draft → Calculate → Approve (locks) → Generate Payslips → Record Payment → Archive, with the Dashboard's "last payroll paid" KPI confirmed to reflect the correct amount afterward.
- **A real gap in an earlier form was caught while testing, not assumed away**: the existing "Add staff" dialog under Settings → People & Access has a required branch-assignment field that isn't obvious from the label alone, a first test attempt silently failed to create a new staff member because of it. Fixed the test, not the form (the requirement itself is reasonable), but worth knowing if a similar issue is ever reported by an actual user.
- **What "integrates with Finance" honestly means here**: there's no unified ledger/Finance module in ImageCare yet (it's not built, Cash Flow and Monthly/Annual Summary are still on the roadmap), so Payroll's Dashboard follows the same KPI-and-quick-action pattern as Bills and Credit for a consistent feel, rather than claiming a shared ledger integration that doesn't exist to integrate with.
- **New roles were not added.** The spec lists Owner/HR Manager/Accountant/Branch Manager/Auditor; the app has one hardcoded "Owner" user and no real multi-user login system (the same limitation noted for every prior module's permissions). `manage_payroll` (which already existed in the permission matrix from earlier work) is reused as-is rather than inventing role distinctions the auth system can't actually enforce.

## Accounting Engine Correction, one shared engine, not per-module math

A mandatory correction applied before continuing finance modules, requiring every module to read financial figures from a single shared engine rather than each computing its own version of "profit" or "cash."

- **A real, previously-invisible data gap was found and fixed first.** Sales already snapshotted the *selling* price at checkout, but never the *buying* price, meaning accurate historical COGS was structurally impossible even to attempt. `SaleLineItem` now snapshots `unitCost` (the product's buying price at the moment of sale) alongside the existing price snapshot, populated in `salesService.checkout()`.
- **`accountingService.ts` is the one engine**, implementing the formulas exactly as specified: `Sales = Σ(Selling Price × Qty)`, `COGS = Σ(Buying Price × Qty)`, `Gross Profit = Sales − COGS`, `Net Profit = Gross Profit − Operating Expenses`. It reads from Sales, Credit, Purchasing, and Expenses, nothing recomputed or duplicated locally by any other module.
- **Cash in Hand is a genuinely separate formula that never touches COGS**, per the spec's explicit warning ("COGS affects profit, not cash"). Verified by code review that no `cogs` reference exists anywhere in `getCashInHandBreakdown()`, not just documented as a rule, checked as fact.
- **A new `CashMovement` ledger was built from scratch** for the three inputs nothing in the app previously tracked: bank deposits, owner withdrawals, and reconciliation adjustments, each requiring a mandatory reason, each permanently logged. A new page (`/cash-movements`) lets an owner record these and see the full Cash in Hand breakdown, line by line, plus an editable Opening Cash balance.
- **The Dashboard now shows all 8 required KPIs** (Today's Sales, Today's COGS, Gross Profit, Today's Expenses, Net Profit, Cash in Hand, Outstanding Credit, Low Stock Alerts), replacing the previous placeholder that simply summed non-credit revenue and called it "cash available."
- **Verified with real numbers, not just code review**: sold 3 units at a known cost and price and confirmed Today's Sales, Today's COGS, and Gross Profit all landed on the exact expected figures; then ran opening cash, a bank deposit, an owner withdrawal, and a positive adjustment through Cash in Hand and confirmed the running total was correct after every single step.
- **Two real, unrelated bugs were caught and fixed while working in this code**: the Dashboard's "New purchase" quick action still said "isn't built yet" even though Purchasing has been complete for several modules now; and a mistake of my own, I first wired "Today's Expenses" to a month-total figure by accident and caught it before it shipped.

## Expenses (IMC-SRS-012), the source of "Operating Expenses" for the accounting engine

- **The full approval workflow is real and tested**: Draft → Submit → Approve/Reject → Paid, with "Approved expenses cannot be deleted" enforced at the service layer (a paid or approved expense genuinely cannot be cancelled, verified by confirming the button itself is unavailable, not just hidden by convention).
- **Categories are entirely business-defined** ("Configurable categories"), no preset list, matching the same pattern used everywhere else in this app.
- **Real file attachments, not a placeholder.** Receipts are read via `FileReader`, stored as base64 in the same offline-first IndexedDB cache everything else uses, and rendered back as an actual image (or a download link for non-images) on the expense's detail page, size-capped at 3MB so a phone photo doesn't quietly bloat the offline store. Verified end-to-end: uploaded a receipt, confirmed it displayed correctly on the detail page afterward.
- **Recurring expenses generate real drafts, not a fake schedule.** A template with a due date in the past, run through "Generate due expenses," produced an actual new draft expense in the register, verified directly, including the same honest pattern used for Loyalty's point expiration: no real background cron exists in this offline-first PWA, so generation is a deliberate, logged action rather than a silent process nobody can see happen.
- **This module is the actual source of "Operating Expenses" for Net Profit and "Business Expenses Paid" for Cash in Hand** in the Accounting Engine Correction above, verified together: a paid expense's amount showed up correctly on both the Expense Dashboard and the main Dashboard's Today's Expenses / Net Profit / Cash in Hand figures in the same test run, not as separate, potentially-inconsistent numbers.
- **Two real gaps from the first pass, caught on a second look and fixed properly:** "Settings" and "Notifications" were both explicitly listed in the spec's core features and integrations, and the first delivery shipped without either, an oversight, not a documented scoping decision, so it's called out here plainly rather than left quiet.
...
## Sales Targets (IMC-SRS-013), the fix that made Staff Targets actually work

- **A real data gap was found and fixed before building the feature on top of it.** Sales were never attributed to any specific staff member, only to a single hardcoded login. Building "Staff Targets" on that foundation would have shipped a feature that looked real but tracked nothing. A "Sold by" field was added to checkout instead, an optional dropdown pulling from Staff Master, so a specific person can be credited for a specific sale. Verified directly: created a staff target, made a sale with that person selected as "Sold by," and watched the target's progress update correctly.
- **Progress is computed live, every time it's viewed**, from real completed sales, never stored and left to drift. Parked and refunded sales are excluded automatically, matching "cancelled sales excluded."
- **Overlapping targets for the same scope and period are blocked**, the same pattern used for Payroll periods, verified by trying to create two overlapping business targets and confirming the second one was rejected.
...
## Cash Flow (IMC-SRS-015), one engine, no second Cash in Hand

The spec required using the shared accounting engine and the IMC Accounting Engine Correction standard for every cash calculation, so this module extends accountingService.ts directly rather than building a second, competing cash system next to it.

- All 6 required Dashboard KPIs (Opening Cash, Cash Received, Cash Paid Out, Cash in Hand, Bank Balance, Net Cash Flow) are built from the exact same breakdown the earlier Cash in Hand work already computes. Verified directly, not just reviewed: recorded a 20,000 UGX bank deposit and confirmed Cash Flow's Dashboard and the older Cash Movements page showed the identical 80,000 UGX cash in hand afterward, proving there is no drift between the two, they are reading the same numbers.
- Cash Ledger is a genuine merge of every cash-affecting source, sales, credit collections, expense payments, supplier payments, and cash movements, into one chronological, running-balance view, filterable to Cash Inflows or Cash Outflows as the spec's two separate core features. Verified: a cash sale appeared correctly under Inflows and was correctly absent from Outflows.
- Bank Balance is honest about what the app can actually track: the only real, recorded flow into the bank is a Bank Deposit movement, so Bank Balance is opening balance plus every deposit ever recorded. Nothing invents a "spend from bank" flow that doesn't exist anywhere else in the app.
- Cash Reconciliation is a real workflow, not a relabeled adjustment form: enter what was physically counted, the system computes the variance against Cash in Hand, and a non-zero variance automatically becomes an audited Cash Adjustment through the same recordCashMovement path everything else uses, while a separate permanent reconciliation record keeps formal end-of-day history distinct from ad-hoc corrections. Verified end to end: counted 5,000 UGX short of the system figure, logged it, and confirmed Cash in Hand updated to the counted amount afterward.
...
## Offline Mode (IMC-SRS-021), the final planned module

This spec asked for several things that are genuinely hard to deliver honestly given what this app actually is: a real login system that does not exist yet, a live multi-device backend that is not connected in this environment, and conflict resolution that structurally needs two disagreeing writers to ever have anything to show. Rather than fake any of that, each piece below is either genuinely built, or clearly labeled for what it actually is.

- **Local encrypted database cache is real, not a label.** Every module in this app funnels through exactly two functions, cacheGet and cacheSet, so that is the one place AES-256-GCM encryption was added, using the browser's own Web Crypto API. This was the highest-risk change made in this entire project, touching the storage layer every one of the 20 prior modules depends on, so it was tested accordingly: created a product, reloaded the page to force a real decrypt from IndexedDB, and confirmed it survived intact; then inspected the raw IndexedDB contents directly and confirmed the plaintext product name did not appear anywhere, only ciphertext did. Backward compatibility was verified the same way: a simulated pre-encryption plaintext record was injected directly into the database and confirmed to still read correctly without crashing, then confirmed it becomes encrypted automatically the moment it's next written, with a manual "Encrypt existing data now" action in Offline Settings for anyone who wants it done immediately rather than waiting on ordinary use.
- **Pending Sync Queue and Sync History are built on the real queue, not a new pretend one.** Every mutation across every module already calls enqueueSync on write; this is the first time that queue is actually surfaced to the owner. Verified end to end: created a product and a sale, confirmed both appeared as real pending items, synced them, confirmed the queue emptied and a permanent history entry was created.
- **Offline Status Dashboard's 8 shared KPIs reuse the exact same accounting and inventory engine every other Dashboard in this app already calls**, not a separate calculation. Verified against a real sale: Sales, COGS, and Gross Profit matched exactly.
- **Conflict Resolution is real infrastructure, honestly empty.** A conflict needs two devices disagreeing about the same record while disconnected from each other, which requires a live, connected, multi-device backend this environment does not have. Nothing here manufactures a fake conflict to look more finished; the page says plainly why the list is empty and what would need to be true for it not to be.
- **What "sync" means here is stated plainly, not implied.** With no live backend to actually push to, syncing marks queued operations processed and logs them, the same honest simulation already disclosed for the sync status indicator elsewhere in this app, not a new, separate claim of real network activity.
- **Flexible, owner-assigned permissions without fixed roles already exists**, built during the Open Staff Roles fix earlier in this project. Nothing needed to be rebuilt here; this module simply relies on that same real, business-defined role catalogue.

With this module, every one of the 21 originally specified modules is implemented.
