...
export const ProductPicker = forwardRef<ProductPickerHandle, ProductPickerProps>(function ProductPicker(
  ...
  // it's not going"). Now backed by NumberField, which already renders an
  // empty field while its underlying value is 0 (see toDisplay() there) and
  // only re-snaps to a valid 1 via `min={1}` on blur - so the cashier can
  // freely clear/retype without fighting a forced "1" mid-keystroke. Nothing
  // about Add-item validation changed: canAdd still requires a real
  // quantity above 0.
