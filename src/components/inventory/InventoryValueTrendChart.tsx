...
export function InventoryValueTrendChart({ data, isLoading, range, onRangeChange, currency }: InventoryValueTrendChartProps) {
  return (
    <Card className="p-5">
        ...
        <div className="flex gap-1 rounded-md bg-ink-50 p-1">
          {RANGES.map((r) => (
            <button
              ...
              className={
                range === r.key
                  ? 'rounded px-2.5 py-1 text-xs font-medium bg-white text-brand-blue-700 shadow-card'
                  : 'rounded px-2.5 py-1 text-xs font-medium text-ink-500 hover:text-ink-900'
              }
            >
              {r.label}
            </button>
      ...
      <p className="mt-2 text-xs text-ink-500">
        Built from real stock movement history — fills in further detail as more activity accumulates.
      </p>
    </Card>
  )
}
